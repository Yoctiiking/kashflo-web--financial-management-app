...
import { getUserProfile, getLastMonthsTransactions, migrateTransactionToSharedBudget } from "@/lib/firebase/firestore";
...
export default function MigrateTransactionModal({ budgetId, onClose, onSuccess }: Props) {
  ...
  // Si la recherche commence par un nom de mois, on l'utilise pour filtrer par période
  ...
  const filtered = transactions.filter(t => {
    ...
    const matchesMonthSearch = parsedSearch === null || (
      t.date.getMonth() === parsedSearch.month &&
      (parsedSearch.year === null || t.date.getFullYear() === parsedSearch.year)
    );

    const matchesDate = dateFilter === "" ||
      format(t.date, "yyyy-MM-dd") === dateFilter;

    return matchesSearch && matchesMonthSearch && matchesDate;
  });
        ...
        <div className="overflow-y-auto flex-1 px-6 pb-6">
          {loading ? (
            ...
            <p className="text-gray-500 text-sm text-center py-8">
              {search || dateFilter ? "Aucun résultat" : "Aucune dépense personnelle"}
            </p>
          ) : (
