...
const SORT_KEYS: SortKey[] = ["name", "status", "role", "createdAt", "lastSignInTime"];
...
export default function AdminPage() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error || t("errors.loadFailed"));
      }
  ...
  const handleDelete = async (target: AdminUser) => {
    ...
    const ok = await confirm({
      ...
      message: t("deleteConfirm.message", { name: target.displayName || target.email || target.uid }),
  ...
  const handleToggleAdmin = async (target: AdminUser) => {
    ...
    const name = target.displayName || target.email || target.uid;
    const ok = await confirm({
      title: nextIsAdmin ? t("promoteConfirm.title") : t("demoteConfirm.title"),
      message: nextIsAdmin
        ? t("promoteConfirm.message", { name })
        : t("demoteConfirm.message", { name }),
      confirmLabel: nextIsAdmin ? t("promoteConfirm.confirm") : t("demoteConfirm.confirm"),
      danger: !nextIsAdmin
    });
    if (!ok) return;
  ...
  const AdminToggleButton = ({ u }: { u: AdminUser }) => {
    ...
    return (
      <button
        ...
        title={u.uid === user?.uid ? t("cannotModifySelf") : undefined}
  ...
  const DeleteButton = ({ u }: { u: AdminUser }) => (
    <button
      ...
      title={u.uid === user?.uid ? t("cannotDeleteSelf") : t("delete")}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-800 text-left text-gray-500">
                    ...
                    <th className="px-5 py-3 font-medium text-right">{t("columns.actions")}</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredUsers.map(u => (
                    <tr key={u.uid} className="border-b border-gray-200 dark:border-gray-800 last:border-0">
                      ...
                      <td className="px-5 py-3 text-gray-600 dark:text-gray-400 whitespace-nowrap">
                        {formatDateTime(u.createdAt, dateTimeLocale)}
                      </td>
                      <td className="px-5 py-3 text-gray-600 dark:text-gray-400 whitespace-nowrap">
                        {formatDateTime(u.lastSignInTime, dateTimeLocale)}
                      </td>
                      <td className="px-5 py-3">
            ...
            {filteredUsers.map(u => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 text-xs mb-3">
                  <div>
                    <div className="text-gray-500 mb-0.5">{t("columns.createdAt")}</div>
                    <div className="text-gray-700 dark:text-gray-300">{formatDateTime(u.createdAt, dateTimeLocale)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500 mb-0.5">{t("columns.lastSignInTime")}</div>
                    <div className="text-gray-700 dark:text-gray-300">{formatDateTime(u.lastSignInTime, dateTimeLocale)}</div>
                  </div>
                </div>
